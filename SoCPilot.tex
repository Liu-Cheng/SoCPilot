...
\begin{itemize}
\item We propose \textbf{SoCPilot}, an application-driven LLM-powered framework that automates the design flow from C/C++ applications to customized heterogeneous SoCs.

\item SoCPilot combines runtime profiling, hardware/software partitioning, accelerator generation, and system-level design space exploration to jointly optimize software execution and hardware acceleration.

\item Experimental results on representative applications demonstrate that SoCPilot can automatically generate and validate FPGA-based SoCs with significant performance improvements over CPU-only execution.
\end{itemize}
...
Table~\ref{tab:e2e_applications} summarizes five representative end-to-end edge applications covering image recognition, document scanning, aerial video analytics, and low-light vision enhancement. Unlike isolated HLS kernels, each benchmark contains multiple dependent functions, CPU-side control code, accelerator candidates with different compute and resource characteristics, and non-negligible data movement overhead. SoCPilot receives the complete C/C++ application, representative inputs, application-level correctness criteria, and performance/resource constraints. Hotspot selection and hardware/software partitioning are then determined by profiling and cost evaluation rather than being manually specified for individual kernels.

  \begin{table*}[t]
      \centering
      \caption{End-to-end edge-application suite. The hotspot column is filled with the functions
      selected by SoCPilot after profiling and cost evaluation.}
      \label{tab:e2e_applications}
      \renewcommand{\arraystretch}{1.12}
      \resizebox{\textwidth}{!}{
      \begin{tabular}{p{2.7cm}p{2.55cm}p{4.85cm}p{4.05cm}p{3.15cm}}
          \toprule
          \textbf{Application} & \textbf{Scenario} & \textbf{Major processing stages} &
          \textbf{Application-level quality metric} & \textbf{Selected hotspot(s)} \\
          \midrule
          License-plate recognition (plate\_recognition) & Roadside or parking camera & Image loading, grayscale
          conversion, morphological enhancement, edge/region extraction, candidate filtering, and plate
          localization & Detection success / bounding-box consistency & \texttt{morphology\_tophat},
          \texttt{morphology\_blackhat} \\
          Color-based object recognition (recognition) & Embedded visual recognition & BGR-to-HSV
          conversion, color mask generation, mask dilation, contour localization, and target-center
          estimation & Recognition accuracy / localization error & \texttt{bgr\_to\_hsv},
          \texttt{dilate\_mask} \\
          Document scanning (scan) & Mobile or edge document scanner & RGB-to-gray conversion, adaptive
          thresholding, morphology, edge extraction, document mask generation, and feature merging &
          Document mask accuracy / binarization quality & \texttt{scan\_adaptive\_threshold} \\
          Wide-area motion imagery (WAMI) & Aerial video analytics & Image debayering, geometric
          warping, gradient-based Lucas--Kanade tracking, Hessian construction, and foreground modeling
          & Tracking error / foreground detection quality & \texttt{warp\_image},
          \texttt{steepest\_descent}, \texttt{hessian} \\
          Night-vision enhancement (nightvision) & Low-light smart camera & Filtering, median denoising,
          histogram equalization, convolutional enhancement, wavelet transform, and image fusion &
          Enhanced-image quality / contrast preservation & \texttt{conv2d} \\
          \bottomrule
      \end{tabular}}
  \end{table*}
...
\begin{table*}[t]
      \centering
      \caption{Execution results of the HSCO-Bench agent-driven flow on our application set.
      All experiments use DeepSeek-v4-pro targeting the ZCU102 platform.
      FPGA synthesis and board execution are skipped using \texttt{--skip-fpga}.}
      ...
      \begin{tabular}{l l r l l p{6.0cm}}
          ...
          Device-ID check &
          BEV-Sim compilation &
          Invalid C++ type operation: \texttt{to\_uint()} is applied to variables of type
          \texttt{uint32\_t}. \\
          ...
          Functional mismatch with the software golden output; 17 output pixels differ from the
          reference results. \\
          ...
          Device-ID check &
          BEV-Sim compilation &
          Invalid C++ type operation: \texttt{to\_int()} is applied to configuration fields of type
          \texttt{int32\_t}. \\
          ...
          BEV-Sim &
          Catapult HLS &
          Variable-division operators cannot be scheduled under the available Catapult resource and
  ...
  \end{table*}
As shown in Table~\ref{tab:hscobench_results}, none of the five 
applications successfully completed the HSCO-Bench validation flow. 
The failures occur at different stages of accelerator generation. 
Nightvision and Plate\_new fail during behavioral-simulation compilation 
due to invalid type conversions in the generated SystemC code, while 
Scan successfully executes the behavioral simulation but fails functional 
verification against the software golden model. Recognition and WAMI 
progress further and pass behavioral simulation, but fail during Catapult 
HLS because of unsupported or difficult-to-schedule operations.

These results indicate that the primary bottleneck is no longer the 
platform configuration or accelerator discovery process, but the quality 
and synthesizability of the LLM-generated accelerator implementations. 
In particular, type correctness, functional equivalence, and HLS-compatible 
arithmetic remain major obstacles for a general-purpose agent-driven 
